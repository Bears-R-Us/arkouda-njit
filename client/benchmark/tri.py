...
def time_ak_tri_graph(lgNv:int, Ne_per_v:int, p:float,directed:int,weighted:int):
    ...
    '''
    start = time.time()
    #Graph=njit.graph_file_read(edges,vertices,2,directed,"../arkouda/data/delaunay/"+filename+"/"+filename+".gr")
    #Graph=njit.graph_file_read(91,20,3,directed,"kang.gr")
    #Graph=njit.graph_file_read(3056,1024,2,directed,"../arkouda/data/"+filename)
    #Graph=njit.graph_file_read(393176,131072,2,directed,"../arkouda/data/"+filename)
    #Graph=njit.graph_file_read(786396,262144,2,directed,"../arkouda/data/delaunay/delaunay_n18.gr")
    #Graph=njit.graph_file_read(3056,1025,2,directed,"data/delaunay_n10.gr")
    #Graph=njit.stream_file_read(3056,1025,2,directed,"data/delaunay_n10.gr",4)
    #triary=njit.stream_tri_cnt(3056,1025,2,directed,"data/delaunay_n10.gr",2)
    #Graph=njit.rmat_gen(lgNv, Ne_per_v, p, directed, weighted)
    #Graph=njit.stream_file_read(103689,8276,2,directed,"data/graphs/wiki")
    #Graph=njit.graph_file_read(2981,2888,2,directed,"data/graphs/fb")
    #Graph=njit.graph_file_read(3,3,2,directed,"data/1.gr")
    #Graph=njit.graph_file_read(6,4,2,directed,"data/2.gr")
    #Graph=njit.graph_file_read(1000,10001,2,directed,"data/line.gr")
    #Graph=njit.graph_file_read(10000,10001,2,directed,"data/10000-1.gr")
    #Graph=njit.graph_file_read(100,101,2,directed,"data/100-1.gr")
    #Graph=njit.graph_file_read(2000,1002,2,directed,"data/2.gr")
    #Graph=njit.graph_file_read(3000,1003,2,directed,"data/3.gr")
    #Graph=njit.graph_file_read(150,53,2,directed,"data/3-50.gr")
    end = time.time()
    '''
    ...
    '''
    print("----------------------")
    print("deparray = njit.graph_bfs(Graph,2)")
    print(deparray)
    print("BFS  the graph takes {:.4f} seconds".format(end-start))
    start = time.time()
    deparray = njit.graph_bfs(Graph,int(Graph.n_vertices)-3)
    end = time.time()
    print("deparray = njit.graph_bfs(Graph,Graph.n_vertices-3)")
    print(deparray)
    print("BFS  the graph takes {:.4f} seconds".format(end-start))
    ll,ver = njit.graph_bfs(Graph,4)
    old=-2;
    visit=[]
    for i in range(int(Graph.n_vertices)):
        cur=ll[i]
