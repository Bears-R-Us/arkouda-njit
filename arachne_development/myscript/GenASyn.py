...
ax.legend(["FastSV","UP","ASynUPS","UPS"])
...
plt.savefig(filename+"-IterNumASynUPS.png")
plt.show()
plt.close()
...
#plt.subplots_adjust(bottom=0.19)
plt.savefig(filename+"-ExeTimeASynUPS.png")
plt.show()
plt.close()
...
ax.legend(["FastSV","UP","ASynUPS","UPS"],ncol=4)
#ax.set_yscale('symlog', basey=2)
plt.yscale('log',base=2) 
plt.ylabel("Speedup")
plt.xlabel("Graph ID")
...
#plt.legend(ncol=4)
#plt.subplots_adjust(bottom=0.19)
plt.savefig(filename+"-SpeedupASyn.png")
plt.show()
plt.close()


all=pd.concat([data, speedup],axis=1)
all.to_csv(filename+"-ASynUPS-all.csv")
#all=pd.concat([data, speedup],axis=1)
