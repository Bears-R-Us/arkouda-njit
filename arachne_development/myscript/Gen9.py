...
iteration_numbers = data.iloc[:, 1:10]
...
ax.legend(["FastSV", "ConnectIt","C-1m1m","C-1","C-1e","C-m", "UP","AUPS","UPS"],ncol=3)
...
plt.savefig(filename+"-IterNum9.png")
plt.show()
plt.close()

execution_values = data.iloc[:, 10:19]
print(execution_values)
exetime=execution_values.to_numpy()
pda=pd.DataFrame(exetime,columns = range(1,10))
...
#plt.subplots_adjust(bottom=0.19)
plt.savefig(filename+"-ExeTime9.png")
plt.show()
plt.close()
...
#ax.legend(["FastSV","UP","UPS"],ncol=3)
ax.legend(["FastSV", "ConnectIt","C-1m1m","C-1","C-1e","C-m", "UP","AUPS","UPS"],ncol=3)
#ax.set_yscale('symlog', basey=2)
plt.yscale('log',base=2) 
plt.ylabel("Speedup")
...
#plt.legend(ncol=4)
#plt.subplots_adjust(bottom=0.19)
plt.savefig(filename+"-Speedup9.png")
plt.show()
plt.close()


all=pd.concat([data, speedup],axis=1)
all.to_csv(filename+"-9-all.csv")
#all=pd.concat([data, speedup],axis=1)
