...
iteration_numbers = data.iloc[:, [1,4,8,9]]
...
plt.savefig(filename+"-IterNumUPS2.png")
plt.show()
plt.close()

execution_values = data.iloc[:, [10,13,17,18]]
...
#plt.subplots_adjust(bottom=0.19)
plt.savefig(filename+"-ExeTimeUPS2.png")
plt.show()
plt.close()
...
ax.legend(["FastSV", "LP","UP","UPS"],ncol=3)
#ax.set_yscale('symlog', basey=2)
plt.yscale('log',base=2) 
plt.ylabel("Speedup")
plt.xlabel("Graph ID")
...
#plt.legend(ncol=4)
#plt.subplots_adjust(bottom=0.19)
plt.savefig(filename+"-SpeedupUPS2.png")
plt.show()
plt.close()
