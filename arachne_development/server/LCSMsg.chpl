module JCSMsg {
  ...
  proc segLCSMsg(cmd: string, msgArgs: borrowed MessageArgs, st: borrowed SymTab): MsgTuple throws {
      ...
      var myoffsegs:[0..0] int =0 ;
      var mystrArray:[0..cur-1]uint(8);

      for i in 0..m-1{
        var mycur:int=0;
        if(common[i]){
          mystrArray[mycur]=string1[i]:uint(8);
          mycur+=1;
        }
      }
      writeln("length of lcs=",mylen,",sequential lcs=",strArray);
...
/*
      ...
      writeln("n=",n);
      ref string1=sstrings1.values.a[0..m-1];
      var mylcs=LL2(string1,string2);
      proc LL2(string1:[?D1] unit(8),string2:[?D2] unit(8)):
      ref string2=sstrings2.values.a[0..n-1];
      var mat:[0..m][0..n] int;
      writeln("string1=",string1);
      writeln("string2=",string2);
