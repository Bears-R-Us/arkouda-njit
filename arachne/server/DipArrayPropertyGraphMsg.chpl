module DipArrayPropertyGraphMsg {
    ...
    */
    proc DipArrayaddNodeLabelsMsg(cmd: string, msgArgs: borrowed MessageArgs, st: borrowed SymTab): MsgTuple throws {
        ...
        var D_byte_label_arrays: domain(2) dmapped Block({0..<label_mapper.size, 0..<node_map.size}) = {0..<label_mapper.size, 0..<node_map.size};
        ...
        forall i in input_vertices.domain {
            ...
            var u = input_vertices[i];
            byte_label_arrays[lbl,u].write(true);
        }
        ...
        graph.withComp(new shared SymEntry2D(byte_label_arrays):GenSymEntry, "DIP_ARR_VERTEX_LABELS");
    ...
    * returns: message back to Python.
    */
    proc DipARRqueryLabelMsg(cmd: string, msgArgs: borrowed MessageArgs, st: borrowed SymTab): MsgTuple throws {
        // Parse the message from Python to extract needed data.
        var graphEntryName = msgArgs.getValueOf("GraphName");
        ...
        var gEntry: borrowed GraphSymEntry = getGraphSymEntry(graphEntryName, st); 
        var graph = gEntry.graph;
        var node_labels = toSymEntry2D(graph.getComp("DIP_ARR_VERTEX_LABELS"), atomic bool).a;
        var node_map = toSymEntry(graph.getComp("NODE_MAP"), int).a;
        ...
        var return_array : [node_map.domain] bool;
        ...
        var timer:stopwatch;
        timer.start();
        forall lbl in labels_to_find_set with (ref return_array) {
            forall (_, node) in node_labels.domain[lbl..lbl, ..] with (ref return_array) {
                if node_labels[lbl,node].read() == true {
                    return_array[node] = true;
                }
            }
        }
        timer.stop();
        var time_msg = "label query DIP-ARR took " + timer.elapsed():string + " sec";
    ...
    use CommandMap;
    registerFunction("DipARRaddNodeLabels", DipArrayaddNodeLabelsMsg, getModuleName());
    ...
    registerFunction("DipARRqueryLabel", DipARRqueryLabelMsg, getModuleName());
}