  ...
  use Logging;
    ...
    class WavefrontState {
        ...
        proc calculatePatternStability(): real {
            const idx = rateIndex.read() % 11;
            if idx < 2 then return 0.0;
            
            var changes = 0;
            for i in 1..min(idx, 3) {  // Look at last 3 points max
                const prevCount = patternDiscoveryRate[(idx-i) % 11][0];
                const currCount = patternDiscoveryRate[(idx-i+1) % 11][0];
                if currCount > prevCount {
                    changes += 1;
                }
            }
            
            return 1.0 - (changes:real / 3.0);
        }
    }
...
proc expandWavefront(ref state: WavefrontState,
    ...
    if randomTarget > 0 {
        ...
        for v in remainingCandidates {
            ...
            if rng.getNext() < 0.3 { // Increased selection probability
                state.wavefront.add(v);
                state.newVertices.add(v);
                selectedRandom += 1;
            }
...
proc processWavefront(ref state: WavefrontState,
    ...
    proc runASWS(n: int, k: int,
        ...
        var maxDeg = max reduce nodeDegree;  // Calculate maxDeg
        var state = new WavefrontState(ASconfig, maxDeg);  // Pass maxDeg
        ...
        while (!state.isSamplingAdequate() && iteration < maxIterations) {
