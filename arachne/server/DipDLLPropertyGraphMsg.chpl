module DipDLLPropertyGraphMsg {
    ...
    */
    proc DipDLLaddNodeLabelsMsg(cmd: string, msgArgs: borrowed MessageArgs, st: borrowed SymTab): MsgTuple throws {
        ...
        forall i in input_vertices.domain with (ref last_label_tracker){
            var lbl = input_labels[i];
            var u = input_vertices[i];
            var new_node = new shared Node(lbl, u);
            node_labels[u].pushBack(new_node);
        ...
        graph.withComp(new shared SymEntry(node_labels):GenSymEntry, "DIP_DLL_VERTEX_LABELS");
    ...
    * returns: message back to Python.
    */
    proc DipDLLqueryLabelMsg(cmd: string, msgArgs: borrowed MessageArgs, st: borrowed SymTab): MsgTuple throws {
        // Parse the message from Python to extract needed data.
        var graphEntryName = msgArgs.getValueOf("GraphName");
        ...
        var gEntry: borrowed GraphSymEntry = getGraphSymEntry(graphEntryName, st); 
        var graph = gEntry.graph;
        var node_labels = toSymEntry(graph.getComp("DIP_DLL_VERTEX_LABELS"), list(shared Node, parSafe=true)).a;
        ...
        forall lbl in labels_to_find_set with (ref return_array) {
            ...
            return_array[borrowed_val.vertex] = true;
    ...
    registerFunction("DipDLLqueryLabel", DipDLLqueryLabelMsg, getModuleName());
}