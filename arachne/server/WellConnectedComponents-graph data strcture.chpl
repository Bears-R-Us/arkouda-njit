...
record Graph {
    var numVertices: int;
    var vertexDomain: domain(int, parSafe=false);
    var edgeDomain: domain(2*int, parSafe=false);        // Stores edges as (v1,v2) where v1 < v2
    var adjList: [vertexDomain] list(int); // Adjacency lists
    ...
    proc addEdge(v1: int, v2: int) {
        ...
        if !edgeDomain.contains((minV, maxV)) {
            ...
            adjList[minV].pushBack(maxV);
            adjList[maxV].pushBack(minV);
            degree[minV] += 1;
            degree[maxV] += 1;
        }
    }
    ...
    proc printGraph() {
        writeln("Graph Information:");
        writeln("Number of vertices: ", numVertices);
        writeln("Number of edges: ", numEdges());
        writeln("Edges:");
        for (v1,v2) in edgeDomain {
            writeln(v1, " -- ", v2);
        }
    }
}
...
record Cut {
    var vertices: domain(int, parSafe=true);    // Vertices in the cut set
    var edges: domain(2*int, parSafe=true);   // Cut edges
    ...
    // proc getComplement(): domain(int, parSafe = true) {
    //     var complement: domain(int, parSafe = true) = {1..totalVertices};
    //     for v in vertices do complement -= v;
    //     return complement;
    // }
    // proc getComplement(): domain(int, parSafe = true) {
    //         var all_vertices: domain(int, parSafe = true);
    //         forall v in 1..totalVertices do all_vertices.add(v);
    //         forall ver in this.vertices do all_vertices.remove(ver);
    //         return all_vertices;
    //     }
    ...
    // }
        ...
        proc getComplement() {
        var complement: domain(int, parSafe=false);  // Create empty associative domain
        for i in 1..totalVertices{
          if !vertices.contains(i) then complement += i;
        }
        return complement;
    }
...
proc scanningPhase(
    ref graph: Graph,
    startVertex: int
): (list(int), [graph.vertexDomain] int) {
    writeln("\n=== Starting scanning phase from vertex ", startVertex, " ===");
    
    var ordering: list(int);
    var inS: [graph.vertexDomain] bool;     // In set S
    var d: [graph.vertexDomain] int;        // Attachment numbers
    var parent: [graph.vertexDomain] int;   // Parent pointers
    
    // Initialize
    d = 0;              // Initialize all attachment numbers to 0
    parent = -1;        // Initialize all parents to -1
    inS = false;        // Initialize all vertices as not in S
    ...
    for neighbor in graph.neighbors(startVertex) {
        d[neighbor] = 1;  // For unweighted graph, attachment number is 1
        writeln("  d[", neighbor, "] = ", d[neighbor]);
    }

    while ordering.size < graph.size() {
        ...
        // Find parent (neighbor in S with maximum weight connection)
        var maxNeighbor = -1;
        var maxAttachment = -1;
        for neighbor in graph.neighbors(maxVertex) {
            if inS[neighbor] && d[neighbor] > maxAttachment {
                maxAttachment = d[neighbor];
        ...
        for neighbor in graph.neighbors(maxVertex) {
            if !inS[neighbor] {
                var oldD = d[neighbor];
                d[neighbor] += 1;  // For unweighted graph, increment by 1
...
proc buildCertificate(
    ...
    // Create new graph for certificate with same number of vertices
    var certificate = new Graph(graph.numVertices);
    
    writeln("Performing ", k, " scanning phases");
    
    // Get array of vertices for start vertex selection
    var vertexArray = graph.vertexDomain.low..graph.vertexDomain.high;
    
    for i in 1..k {
        var startVertex = vertexArray[(i - 1) % graph.numVertices];
        writeln("\nPhase ", i, " starting from vertex ", startVertex);
        
        var (ordering, parent) = scanningPhase(graph, startVertex);

        // Add edges from the forest to certificate
        for v in graph.vertexDomain {
            var p = parent[v];
            if p != -1 {
                certificate.addEdge(v, p);
                writeln("Added edge ", v, " -- ", p, " to certificate");
            }
        }
    }
    ...
    for v in certificate.vertexDomain {
        writeln(v, " -> ", certificate.neighbors(v));
    }
...
*/
/* Find all minimum cuts using new Graph record */
proc findAllMinCutsNI(
    ...
    for startVertex in graph.vertexDomain {
        ...
        // Track vertices in set S using domain for efficiency
        var S: domain(int, parSafe=true);
        ...
        for idx in 0..(ordering.size - 2) {
            ...
            // Add vertices to cut
            for u in S do
                currentCut.addVertex(u);
            
            // Find cut edges efficiently using graph structure
            for u in S {
                for neighbor in graph.neighbors(u) {
                    if !S.contains(neighbor) {
                        var (minV, maxV) = if u < neighbor then (u, neighbor) else (neighbor, u);
                        
                        // Only add edge if not already counted
                        if !currentCut.edges.contains((minV, maxV)) {
                            currentCut.addEdge(minV, maxV);
                            cutWeight += 1;
                        }
                    }
                }
            ...
            currentCut.value = cutWeight;
            
            writeln("Cut value for set ", S, " is ", cutWeight, " (ratio: ", currentCut.ratio, ")");
            ...
            if cutWeight <= globalMinValue {
                ...
                allCuts.pushBack(currentCut);
                writeln("Added cut: ", S, " with edges ", currentCut.edges);
                
                // Create and add complement cut
                if 2 * S.size != graph.numVertices {  // Don't add complement if it's the same size
                    var complementCut = new Cut(graph.numVertices);
                    for v in graph.vertexDomain {
                        if !S.contains(v) then
                            complementCut.addVertex(v);
                    }
                    // Use same edges and cut value
                    complementCut.edges = currentCut.edges;
                    complementCut.value = cutWeight;
                    allCuts.pushBack(complementCut);
                    writeln("Added complement cut: ", complementCut.vertices, " with edges ", complementCut.edges);
                }
            }
        }
    }
    ...
    // Handle balance ratio filtering if specified
    if balanceRatio > 0.0 {
        writeln("\nFiltering cuts based on balance ratio ", balanceRatio);
        var bestBalanceScore = max(real);
        var balancedCuts: list(Cut);

        // Find best balance score
        for cut in allCuts {
            var balanceScore = cut.getRatioDifference(balanceRatio);
            bestBalanceScore = min(bestBalanceScore, balanceScore);
        }

        // Keep cuts with best balance
        for cut in allCuts {
            var balanceScore = cut.getRatioDifference(balanceRatio);
            if abs(balanceScore - bestBalanceScore) < 1e-10 {
                balancedCuts.pushBack(cut);
                writeln("Selected balanced cut: ", cut.vertices, " (ratio: ", cut.ratio, ")");
            }
        }

        writeln("\nFound ", balancedCuts.size, " balanced minimum cuts:");
        for cut in balancedCuts {
            cut.printInfo();
        }
    ...
    writeln("\nFound ", allCuts.size, " minimum cuts:");
    for cut in allCuts {
        cut.printInfo();
    }

    return (globalMinValue, allCuts);
}
...
proc testNagamochiIbaraki() throws {
    writeln("=== Testing Nagamochi-Ibaraki Algorithm with New Graph Structure ===\n");

    // Test 1: Graph Data Structure Basic Operations
    {
        writeln("Test 1: Testing Graph Data Structure Operations");
        var g = new Graph(5);  // Create graph with 5 vertices
...
/*        
        ...
        assert(g.numEdges() == 3);
        writeln("  Vertices: ", g.numVertices, " (expected: 5)");
        writeln("  Edges: ", g.numEdges(), " (expected: 3)");
        
        // Test neighbor access
        writeln("\nTesting neighbor access:");
        var neighbors1 = g.neighbors(1);
        writeln("  Neighbors of 1: ", neighbors1, " (expected: 2,3)");
        assert(neighbors1.size == 2);
        
        // Test edge existence
        writeln("\nTesting edge existence:");
        assert(g.hasEdge(1, 2));
        assert(g.hasEdge(2, 1));  // Should work in both directions
        assert(!g.hasEdge(1, 4));
        writeln("  Edge checks passed");
        
        // Test degree information
        writeln("\nTesting degree information:");
        assert(g.degree[1] == 2);
        assert(g.degree[4] == 0);
        writeln("  Degree checks passed");
        
        writeln("\nGraph Data Structure Tests Passed!");
    }

    // Test 2: Simple cycle graph (4 vertices)
    {
        writeln("\nTest 2: Simple cycle graph (4 vertices)");
        var g = new Graph(4);
        ...
        // Test algorithm
        var (minCutValue, cuts) = findAllMinCutsNI(g);
        
        writeln("\nResults for cycle graph:");
        assert(minCutValue == 2, "Minimum cut should be 2");
        assert(cuts.size == 8, "Should find 8 minimum cuts");  // All possible 2-edge cuts
        writeln("  Minimum cut value: ", minCutValue, " (expected: 2)");
        writeln("  Number of cuts found: ", cuts.size, " (expected: 8)");
    }
    ...
    // Test 3: Complete graph (4 vertices)
    {
        ...
        assert(minCutValue == 3, "Minimum cut should be 3");
        writeln("  Minimum cut value: ", minCutValue, " (expected: 3)");
    }
    ...
    // Test 4: Bridge graph
    {
        ...
        assert(minCutValue == 1, "Minimum cut should be 1 (bridge)");
        writeln("  Minimum cut value: ", minCutValue, " (expected: 1)");
    }

    // Test 5: Balanced Cut Test
    {
        writeln("\nTest 5: Balanced Cut Test (with ratio 0.5)");
        var g = new Graph(6);
        
        // Create a graph with clear balanced cut
        // Left side: 1-2-3 fully connected
        g.addEdge(1, 2);
        g.addEdge(2, 3);
        g.addEdge(1, 3);
        
        // Right side: 4-5-6 fully connected
        g.addEdge(4, 5);
        g.addEdge(5, 6);
        g.addEdge(4, 6);
        
        // Two edges between sides
        g.addEdge(1, 4);
        g.addEdge(3, 6);
        ...
        var (minCutValue, cuts) = findAllMinCutsNI(g, 0.5);  // Look for balanced cuts
        
        writeln("\nResults for balanced cut test:");
        writeln("  Minimum cut value: ", minCutValue);
        writeln("  Number of balanced cuts found: ", cuts.size);
        
        // Verify that cuts are balanced
        for cut in cuts {
            var ratio = cut.ratio;
            assert(abs(ratio - 0.5) < 0.1, "Cut should be approximately balanced");
            writeln("  Cut ratio: ", ratio, " (should be close to 0.5)");
        }
    }

    // Test 6: Large Dense Graph Performance Test
    {
        writeln("\nTest 6: Large Dense Graph Performance (20 vertices)");
        var g = new Graph(20);
        
        // Add edges with 80% density
        var numPossibleEdges = (20 * 19) / 2;
        var targetEdges = (numPossibleEdges * 8) / 10;  // 80% of possible edges
        ...
        writeln("\nCreated dense graph with:");
        writeln("  Vertices: ", g.numVertices);
        writeln("  Edges: ", g.numEdges());
        writeln("  Density: ", (g.numEdges():real / numPossibleEdges:real):string, "%");
        ...
        t1.stop();
        writeln("\nPerformance results:");
        writeln("  Time taken: ", t1.elapsed(), " seconds");
        writeln("  Minimum cut value: ", minCutValue);
        writeln("  Number of cuts found: ", cuts.size);
    }
    */
}
