...
if __name__ == "__main__":
    ...
    file_path = '/scratch/users/md724/DataSets/sx-mathoverflow/sx-mathoverflow.txt'

    # Load the data into a Pandas DataFrame
    mathoverflow = pd.read_csv(file_path, sep=' ', header=None)


    # Assign column names
    mathoverflow.columns = ['src', 'dst', 'UNIXTS']

    mathoverflow['type'] = 'T1'

    print(mathoverflow.head())
    # Concatenate 'src' and 'dst' columns and find unique elements for nodes
    unique_nodes = pd.concat([mathoverflow['src'], mathoverflow['dst']]).unique()

    # Number of unique nodes
    num_unique_nodes = len(unique_nodes)

    # Number of edges is the length of the DataFrame
    num_edges = len(mathoverflow)
    ...
    ak_mathoverflow= ak.DataFrame(mathoverflow)
 
    ar_mathoverflow = ar.PropGraph()
    ar_mathoverflow.load_edge_attributes(ak_mathoverflow, source_column="src", destination_column="dst", relationship_columns=["type"])

    all_nodes = ak.concatenate([ak_mathoverflow['src'],ak_mathoverflow['dst']])
    unique_nodes = ak.unique(all_nodes)
    #unique_nodes.size
    lbls = ak.array(["1"]*unique_nodes.size)
    mathoverflow_node_df = ak.DataFrame({"nodes": unique_nodes, "lbls":lbls})

    ar_mathoverflow.load_node_attributes(mathoverflow_node_df,node_column="nodes", label_columns=["lbls"])
    ...
    src_subgraph = ak.array([1, 1, 2])
    dst_subgraph = ak.array([0, 2, 0])
    lbls_subgraph = ak.array(["1"]*3)
    rels_subgraph = ak.array(["T1"]*len(src_subgraph))
    print("len(src_subgraph) = ", len(src_subgraph))
