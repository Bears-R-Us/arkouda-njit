...
SETUP_SCRIPT=./setDistributedChapel
SERVER_PORT=5555
NUM_TRIALS=3
LOG_DIR=/scratch/users/md724/arkouda-njit/arachne/benchmarks/logs-DotMotif-Paper-/
...
run_experiment() {
    ...
    cd $ARKOUDA_DIR
    ./arkouda_server -nl 1 >& ${LOG_DIR}arkouda_server_log.txt &
    SERVER_PID=$!

    # Check server readiness
    if ! check_server_ready "${LOG_DIR}arkouda_server_log.txt"; then
    ...
    for i in $(seq 1 $NUM_TRIALS); do
        ...
        python3 subgraph_isomorphism_DotMotif.py $SERVER_NODE $SERVER_PORT 1 1 >& ${LOG_DIR}benchmark_trial_${i}.txt
    done
