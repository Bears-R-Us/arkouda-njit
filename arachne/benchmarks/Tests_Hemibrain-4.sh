...
PYTHON_SCRIPT=subgraph_isomorphism_Hemibrain-4.py
SERVER_PORT=5555
NUM_TRIALS=3
LOG_DIR=/scratch/users/md724/arkouda-njit/arachne/benchmarks/logs-Hemibrain-4/
...
run_experiment() {
    ...
    cd $ARKOUDA_DIR
    ./arkouda_server -nl 1 >& ${LOG_DIR}arkouda_server_log4.txt &
    SERVER_PID=$!

    # Check server readiness
    if ! check_server_ready "${LOG_DIR}arkouda_server_log4.txt"; then
    ...
    for i in $(seq 1 $NUM_TRIALS); do
        ...
        python3 $PYTHON_SCRIPT $SERVER_NODE $SERVER_PORT 50000 1 1 >& ${LOG_DIR}benchmark_trial4_${i}.txt
    done
