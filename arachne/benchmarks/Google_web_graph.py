...
if __name__ == "__main__":
    ...
    Google_web = pd.read_csv("/scratch/users/md724/DataSets/Google_web_graph/web-Google.txt", sep='\t', comment='#', names=['FromNodeId', 'ToNodeId'])

    # Rename the columns
    Google_web = Google_web.rename(columns={'FromNodeId': 'src', 'ToNodeId': 'dst'})
    Google_web['type'] = 'T1'

    print(Google_web.head())
    # Concatenate 'src' and 'dst' columns and find unique elements for nodes
    unique_nodes = pd.concat([Google_web['src'], Google_web['dst']]).unique()

    # Number of unique nodes
    num_unique_nodes = len(unique_nodes)

    # Number of edges is the length of the DataFrame
    num_edges = len(Google_web)
    ...
    ak_Google_web= ak.DataFrame(Google_web)
 
    ar_Google_web = ar.PropGraph()
    ar_Google_web.load_edge_attributes(ak_Google_web, source_column="src", destination_column="dst", relationship_columns=["type"])

    all_nodes = ak.concatenate([ak_Google_web['src'],ak_Google_web['dst']])
    unique_nodes = ak.unique(all_nodes)
    #unique_nodes.size
    lbls = ak.array(["1"]*unique_nodes.size)
    Google_web_node_df = ak.DataFrame({"nodes": unique_nodes, "lbls":lbls})

    ar_Google_web.load_node_attributes(Google_web_node_df,node_column="nodes", label_columns=["lbls"])
