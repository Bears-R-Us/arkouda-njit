...
def create_random_directed_graph(num_nodes, p):
    ...
    return src, dst
def grf_to_graph(input_file):
    G = nx.DiGraph()  # Create a directed graph
    src = []  # List to store the source nodes of edges
    dst = []  # List to store the destination nodes of edges

    with open(input_file, 'r') as f:
        # Read the number of nodes
        num_nodes = int(f.readline().strip())

        # Read each node and its label (labels are ignored in this example, assuming they are always 1)
        for _ in range(num_nodes):
            node, label = f.readline().strip().split()  # Read the node and label
            G.add_node(int(node))  # Add node to the graph

        # Read edges for each node
        for node in range(num_nodes):
            num_edges = int(f.readline().strip())  # Read the number of outgoing edges
            for _ in range(num_edges):
                source, target = f.readline().strip().split()  # Read each edge
                G.add_edge(int(source), int(target))  # Add edge to the graph
                src.append(int(source))  # Add to source array
                dst.append(int(target))  # Add to destination array
...
if __name__ == "__main__":
    ...
    input_file = 'mdmain_110000_0005.grf'
    G, src, dst = grf_to_graph(input_file)
