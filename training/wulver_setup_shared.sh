#!/bin/bash
# wulver_setup_shared.sh — Automated setup of Arkouda + Arachne on Wulver HPC
#                          (shared memory / single-node build)
#
# PREREQUISITES:
...
#
# USAGE:
#   source training/wulver_setup_shared.sh          # run all phases
#   source training/wulver_setup_shared.sh --help   # show options
#
...
for arg in "$@"; do
    case $arg in
        ...
        --help)
            echo "Usage: source wulver_setup_shared.sh [--skip-chapel] [--skip-arkouda]"
...
if [ "$SKIP_CHAPEL" = false ]; then
    phase "Building Chapel ${CHAPEL_VERSION} (shared memory)"
    ...
    export CHPL_TARGET_CPU=native CHPL_TARGET_MEM=jemalloc CHPL_HOST_MEM=mimalloc
fi
